...
- Lag-lift ranking Action (`trigger tag × condition`, baseline vs days 0–N after, sample sizes). Default window **0–3** and the whole lag profile out to a week, not a single `N`; below ~**90 days** carrying both a rating and a logged meal it returns an explicit insufficient-data state rather than a ranking (SUI-36, D31).
