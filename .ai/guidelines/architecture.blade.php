...
- **Every day-scoped write carries `WithinJournalBounds`.** The `{date}` route segment matches any
  four-digit year, and the insights read is linear in first rating to last rating, so an unbounded
  date is an unbounded page (D33). A new route under `day/{date}` folds the date in from the route
  and validates `['bail', 'required', 'date_format:Y-m-d', new WithinJournalBounds($user)]`. The
  bound stays at the request edge — the Actions also write history for seeders and the suite.
  Removal paths are deliberately exempt: the days they most need to reach are the refused ones.
